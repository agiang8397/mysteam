...
const getAllGames = async () => {
  try {
    const res = await fetch(
      "https://steam-api-dot-cs-platform-306304.et.r.appspot.com/games?limit=50",
      requestOptions
    );
...
const getSearchGames = async () => {
  try {
    const res = await fetch(
      `https://steam-api-dot-cs-platform-306304.et.r.appspot.com/games?limit=50&q=${input.value}`,
      requestOptions
    );
    const datagames = await res.json();
    return datagames.data;
